...
//	  }
	 ...
	 else if (rBuffer[0] == '$') //ignore  msg  from other target
    {
    Clean_InputData();
    delay(300);
	return;
...
Serial.end();
	  ...
	  III_Control_LED(0);
	   ...
	   Clean_InputData();
		 ...
		 int loops=_atoi(rBuffer[1]);
		 int isss;
			 for(isss=0;isss<loops;isss++)
		   {
			 III_Control_LED(0);
			 delay(50); 	 
			 III_Control_LED(1);
